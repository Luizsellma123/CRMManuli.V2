...
{
    public class AtualizacaoIntegracaoPedidoController : ApiController
    {
        public HttpResponseMessage GetAllPedidoAtualizacaoIntegracaoPedidos()
        {
            return Request.CreateErrorResponse(HttpStatusCode.Created, "Nada a retornar");
        }

        public HttpResponseMessage PostPedidoAtualizacaoIntegracaoPedidos(PedidoInclusaoModel OBJPedido)
        {
            RetornoClass OBJRetorno = new RetornoClass();

            try
            {
                OBJRetorno.MsgRetorno = OBJPedido.AtualizaIntegracaoPedido();
