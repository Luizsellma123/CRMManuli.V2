...
{
    public class ClienteAtualizarVendedorController : ApiController
    {
        public HttpResponseMessage GetAllClienteAtualizarVendedor()
        {
            return Request.CreateErrorResponse(HttpStatusCode.Created, "Nada a retornar");
        }

        public HttpResponseMessage PostClienteAtualizarVendedor(CadastroClienteModel OBJCliente)
        {
            RetornoClass OBJRetorno = new RetornoClass();

            try
            {
                OBJRetorno.MsgRetorno = OBJCliente.AtualizaClienteVendedor();
