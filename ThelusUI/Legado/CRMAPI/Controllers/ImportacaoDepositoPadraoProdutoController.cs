...
{
    public class ImportacaoDepositoPadraoProdutoController : ApiController
    {
        ...
        public HttpResponseMessage ImportacaoEstoqueDepositos(Models.ProdutoClass OBJProduto)
        {
            RetornoClass OBJRetorno = new RetornoClass();

            try
            {
                OBJRetorno.MsgRetorno = OBJProduto.ImportacaoDepositoPadraoProdutos();

                var response = Request.CreateResponse<RetornoClass>(HttpStatusCode.Created, OBJRetorno);
                string uri = Url.Link("DefaultApi", new { id = OBJProduto.CodigoProdutoSAP });
