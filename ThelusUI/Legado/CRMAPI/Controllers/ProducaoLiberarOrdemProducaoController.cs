...
{
    public class ProducaoLiberarOrdemProducaoController : ApiController
    {
        public HttpResponseMessage GetAllLiberarOrdemProducao()
        {
            return Request.CreateErrorResponse(HttpStatusCode.Created, "Nada a retornar");
        }

        public HttpResponseMessage PostLiberarOrdemProducao(ProducaoModel OBJProducao)
        {
            RetornoClass OBJRetorno = new RetornoClass();

            try
            {
                OBJRetorno.MsgRetorno = OBJProducao.LiberaOrdemProducao();
