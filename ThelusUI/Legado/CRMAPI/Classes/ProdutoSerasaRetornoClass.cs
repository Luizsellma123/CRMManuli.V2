...
{
    public class ProdutoSerasaRetornoClass : ConexaoClass
    {
        ...
        public List<ProdutoSerasaRetornoFilhoClass> ProdutoSerasaRetornoFilho = new List<ProdutoSerasaRetornoFilhoClass>();

        public string RecuperaConfiguracaoRetornoProdutoFilho()
        {
            ...
            try
            {
                using (SqlConnection dbConnection = new SqlConnection(strConec))
                {
                    ...
                    SqlCommand dbCommand = new SqlCommand("CRM_SP_API_SERASA_CONFIG_PROD_RETORNO_FILHO", dbConnection);
                    ...
                    using (SqlDataReader dataReader = dbCommand.ExecuteReader())
                    {
                        ...
                        if (outputTable.Rows.Count > 0)
                        {
                            foreach (DataRow row in outputTable.Rows)
                            {
                                if (erro == "")
                                {
                                    ProdutoSerasaRetornoFilhoClass OBJProdutoSerasaRetornoFilho = new ProdutoSerasaRetornoFilhoClass();
                                    ...
                                    ProdutoSerasaRetornoFilho.Add(OBJProdutoSerasaRetornoFilho);
