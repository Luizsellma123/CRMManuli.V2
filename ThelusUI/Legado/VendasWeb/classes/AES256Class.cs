...
namespace VendasWeb.classes
{
    public class AES256Class
    {
        ...
        public string Encrypt_V2(byte[] data, string passphrase)
        {
            using (var random = new RNGCryptoServiceProvider())
            {
                // Gera um sal aleatório de 8 bytes
                byte[] salt = new byte[8];
                random.GetBytes(salt);

                // Deriva a chave e o IV usando o sal
                DeriveKeyAndIv(passphrase, salt);

                byte[] encrypted;
                using (var aes = new AesManaged()) // Substitua RijndaelManaged por AesManaged
                {
                    aes.BlockSize = 128; // Tamanho do bloco em bits (128 bits para AES)
                    aes.Mode = CipherMode.CBC;
                    aes.Padding = PaddingMode.PKCS7;
                    aes.Key = key;
                    aes.IV = iv;
                ...
                // Concatena "Salted__", o sal e o texto criptografado
                byte[] saltedEncrypted = Concat(Concat(Encoding.UTF8.GetBytes("Salted__"), salt), encrypted);
                return Convert.ToBase64String(saltedEncrypted);
            }
        }

        public byte[] DecryptToBytes_V2(string encrypted, string passphrase)
        {
            byte[] ct = Convert.FromBase64String(encrypted);
            if (ct == null || ct.Length <= 16) // 8 bytes for "Salted__" + 8 bytes for salt
            {
                return new byte[0];
            }
            ...
            byte[] cipherText = new byte[ct.Length - 16];
            Array.Copy(ct, 16, cipherText, 0, cipherText.Length);
            ...
            {
                aes.BlockSize = 128; // AES block size in bits (128 bits = 16 bytes)
                aes.Mode = CipherMode.CBC;
                aes.Padding = PaddingMode.PKCS7;
                aes.Key = key;
                aes.IV = iv;
