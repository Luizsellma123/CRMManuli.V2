    ...
    public partial class cadPedidoArte : System.Web.UI.Page
    {
        ...
        public string gerarLista(int quant)
        {
            ...
            if (tipoConsulta == 1)
            {
                ...
                strSQL += "select count(*) as CNT from CRM_PRODUTO CP INNER JOIN ";

                strSQL += "CRM_PRODUTO_GRUPO CPG ON CP.IDGrupo = CPG.IDGrupo and (CPG.Nome = 'PROD - CLICHÊ' OR CPG.Nome = 'PROD - INSUMOS') ";
                
                strSQL += "WHERE CP.Nome like '%" + valorConsulta.ToString() + "%' ";

                strSQL += "and (Exists (select * from CRM_PRODUTO_GRUPO CPG2 ";
                strSQL += "INNER JOIN CRM_PRODUTO CP2 ON CPG2.IDGrupo=CP2.IDGrupo and CPG2.Nome = 'PROD - CLICHÊ' ";
                strSQL += "INNER JOIN CRM_PRODUTO_CLIENTE CPC ON CPC.IDProduto=CP2.IDProduto ";
                strSQL += "Where CPC.IDCliente = ' " + novoPedido.codigoEntidade + "' and CP2.IDProduto = CP.IDProduto) ";
                strSQL += "OR CPG.Nome = 'PROD - INSUMOS') ";
            }
            else
            {
                ...
                strSQL += "WHERE CP.CodigoProdutoSAP like '%" + valorConsulta.ToString() + "%' ";
        ...
        public string linhasProduto(int indexPage, int fimPage)
        {
            ...
            if (tipoConsulta == 1)
            {
                ...
                strSQL += "select CP.CodigoProdutoSAP ProdCodEstr, CP.Nome ProdNome, ROW_NUMBER() OVER(ORDER BY CP.CodigoProdutoSAP) as reg ";
                strSQL += "from CRM_PRODUTO CP INNER JOIN ";
