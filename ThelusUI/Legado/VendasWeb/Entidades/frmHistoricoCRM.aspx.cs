...
namespace VendasWeb.Entidades
{
    public partial class frmHistoricoCRM : System.Web.UI.Page
    {
        ...
        GerencialVendas.HistoricoCRMClass HistoricoCRMClass = new GerencialVendas.HistoricoCRMClass();
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                if (Session["TelaHist"].ToString() == "Incluir" || Session["TelaHist"].ToString() == "Excluir")
                {
                    drpEvento.SelectedValue = "10";
                ...
                HistoricoCRMClass.UsuCod = clsEntidades.UsuCod;
        ...
        public void Atualizar_Grid()
        {
            ...
            System.Text.StringBuilder Historico = new System.Text.StringBuilder();

            if (outputTable.Rows.Count > 0)
            {
                foreach (DataRow row in outputTable.Rows)
                {
                    //Historico.AppendFormat();

                    Historico.AppendFormat("<div class=\"timeline-entry\"> <div class=\"timeline-stat\"> ");

                    string evento = row["Evento"].ToString();

                    string dataCad = row["DataCad"].ToString();

                    string categoria = row["Categoria"].ToString();

                    string usuCod = row["UsuCod"].ToString();

                    string historico = row["Historico"].ToString();

                    string dataAgenda = row["DataAgenda"].ToString();

                    switch (evento)
                    {
                        case "Atendimento":
                            Historico.AppendFormat("<div class=\"timeline-icon bg-info\"><i class=\"fa fa-comment-o fa-lg\"></i> ");
                            break;

                        case "Visita Teste":
                            Historico.AppendFormat("<div class=\"timeline-icon bg-primary\"><i class=\"fa fa-car fa-lg\"></i> ");
                            break;

                        case "Negociação":
                            Historico.AppendFormat("<div class=\"timeline-icon bg-dark\"><i class=\"fa fa-comments-o fa-lg\"></i> ");
                            break;

                        case "Venda Fechada":
                            Historico.AppendFormat("<div class=\"timeline-icon bg-success\"><i class=\"fa fa-thumbs-o-up fa-lg\"></i> ");
                            break;

                        case "Venda Perdida":
                            Historico.AppendFormat("<div class=\"timeline-icon bg-danger\"><i class=\"fa fa-thumbs-down fa-lg\"></i> ");
                            break;

                        case "Outros":
                            Historico.AppendFormat("<div class=\"timeline-icon bg-gra\"><i class=\"fa fa-plus-square-o fa-lg\"></i> ");
                            break;

                        case "Pedido":
                            Historico.AppendFormat("<div class=\"timeline-icon bg-mint\"><i class=\"fa fa-shopping-cart fa-lg\"></i> ");
                            break;

                        case "Nota":
                            Historico.AppendFormat("<div class=\"timeline-icon bg-warning\"><i class=\"fa fa-pencil-square-o fa-lg\"></i> ");
                            break;

                        case "Observações":
                            Historico.AppendFormat("<div class=\"timeline-icon bg-purple\"><i class=\"fa fa-warning fa-lg\"></i> ");
                            break;

                        case "Mudança":
                            Historico.AppendFormat("<div class=\"timeline-icon bg-pink\"><i class=\"fa fa-random fa-lg\"></i> ");
                            break;
                    }

                    Historico.AppendFormat("</div><div class=\"timeline-time\"><b>"
                                          + dataCad + "</b></div> "
                                          + "</div><div class=\"timeline-label\"> ");

                    if (evento == "Atendimento")
                    {
                        Historico.AppendFormat("<p class=\"mar-no pad-btm\"> <span class=\"badge badge-info\">" + evento + " via ");

                        switch (categoria)
                        {
                            case "Telefone":
                                Historico.AppendFormat("<i class=\"fa fa-phone\"></i> " + categoria + "</span>");
                                break;

                            case "E-mail":
                                Historico.AppendFormat("<i class=\"fa fa-envelope\"></i> " + categoria + "</span>");
                                break;

                            case "Visita":
                                Historico.AppendFormat("<i class=\"fa fa-car\"></i> " + categoria + "</span>");
                                break;

                            case "Online":
                                Historico.AppendFormat("<i class=\"fa fa-desktop\"></i> " + categoria + "</span>");
                                break;
                        }
                    ...
                    {
                        Historico.AppendFormat("<p class=\"mar-no pad-btm\"> ");

                        switch (evento)
                        {
                            case "Visita Teste":
                                Historico.AppendFormat("<span class=\"badge badge-primary\">");
                                break;

                            case "Negociação":
                                Historico.AppendFormat("<span class=\"badge badge-dark\">");
                                break;

                            case "Venda Fechada":
                                Historico.AppendFormat("<span class=\"badge badge-success\">");
                                break;

                            case "Venda Perdida":
                                Historico.AppendFormat("<span class=\"badge badge-danger\">");
                                break;

                            case "Outros":
                                Historico.AppendFormat("<span class=\"badge badge-gray\">");
                                break;

                            case "Pedido":
                                Historico.AppendFormat("<span class=\"badge badge-mint\">");
                                break;

                            case "Nota":
                                Historico.AppendFormat("<span class=\"badge badge-warning\">");
                                break;

                            case "Observações":
                                Historico.AppendFormat("<span class=\"badge badge-purple\">");
                                break;

                            case "Mudança":
                                Historico.AppendFormat("<span class=\"badge badge-pink\">");
                                break;
                        }

                        Historico.AppendFormat(evento + " " + categoria);

                    }


                    Historico.AppendFormat("</span> por <a href=\"#\" class=\"btn-link btn-md text-semibold\"> "
                                                          + usuCod + "</a></p>"
                    ...
                    if (dataAgenda != "")
                    {
                        Historico.AppendFormat("<p class=\"text-default mar-no pad-top\"><i class=\"fa fa-clock-o\"></i> Agendamento para novo contato - " + row["DataAgenda"].ToString() + "</p>");
                    }

                    Historico.AppendFormat("</div></div>");
                }
            }

            lblHistorico.Text = Historico.ToString();
        }
        ...
        */
        ...
        protected void SalvarButton_Click(object sender, EventArgs e)
        {
            ...
            {
                //Response.Write("<script>alert(\"Obrigatório informar a categoria e o evento.\");</script>");
                Session["Msg"] = "Obrigatório informar a categoria e o evento.";
            }
            else
            {
                string erro = "";
                    ...
                    if (erro == "")
                    {
                        ...
                        switch (clsEntidades.TipoOperacao)
                        {
                            case "Incluir Carteira":
                                ...
                                VendedorClass.CodigoClienteSAP = clsEntidades.CodigoClienteSAP;
                                VendedorClass.VendEntPrinc = "Não";

                                if (VendedorClass.VendCod != "" && VendedorClass.VendCod != null)
                                {
                                    erro = VendedorClass.Incluir_Vend_Ent();
                                    clsEntidades.TipoOperacao = "";
                                    Session["clsEntidades"] = clsEntidades;
                                    if (erro == "")
                                    {
                                        //Response.Write("<script>alert(\"Cliente vinculada com sucesso.\");</script>");
                                        Session["Msg"] = "Cliente vinculado com sucesso.";
                                    }
                                    else
                                    {
                                ...
                                {
                                    //Response.Write("<script>alert(\"Sem Vendedor selecionado, verifique.\");</script>");
                                    Session["Msg"] = "Sem Vendedor selecionado, verifique.";
                                }
                                break;
                            ...
                            case "Excluir Carteira":
                                ...
                                if (erro == "")
                                {
                                    //Response.Write("<script>alert(\"Entidade desvinculada com sucesso.\");</script>");
                                    Session["Msg"] = "Entidade desvinculada com sucesso.";
                                }
                                else
                                {
                                    //Response.Write("<script>alert(\"" + erro + "\");</script>");
                            ...
                            default:
                                clsEntidades.TipoOperacao = "";
                                Session["clsEntidades"] = clsEntidades;
                                //Response.Write("<script>alert(\"Histórico inserido com sucesso.\");</script>");
                                Session["Msg"] = "Histórico inserido com sucesso.";
                                break;
                        }
                ...
                {
                    Session["TemHistorico"] = "Não";
                    //Response.Write("<script>alert(\"Histórico deve ter no mínimo 20 caracteres.\");</script>");
                    Session["Msg"] = "Histórico deve ter no mínimo 20 caracteres.";
                }
            }

            //Verifica se tem msg para apresentar
            if (Session["Msg"] != null)
            {
                ///Response.Write("<script>alert(\"" + Session["Msg"].ToString() + "\");</script>");
                ((Label)Master.Master.FindControl("MenssagemMasterLabel")).Text = ObjUtilClass.MenssagemAlerta(Session["Msg"].ToString(), true);
