...
{
    public partial class FrmListaPedidos : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                CarregaDadosNaTelaIsPostBack();                
            }
            else
            {
                CarregaDadosNaTelaIsNotPostBack();
            }
        }
        ...
        protected void CarregaDadosNaTelaIsPostBack()
        {
            ...
            if (Session["ObjFiltroClass"] != null)
            {
                ...
                drpListFiltroStat.SelectedValue = ObjFiltroClass.PedVendaStatDescr;
        ...
        protected void CarregaDadosNaTelaIsNotPostBack()
        {
        ...
        public void Atualizar_Grid()
        {
            ...
            switch (drpListFiltroPri.SelectedValue.ToString())
            {
                ...
                case "5":
                    PedidoClass.CodigoProdutoSAP = txtFiltro.Text;
                    break;

                case "6":
                    PedidoClass.PedVendaNumSAP = txtFiltro.Text;
                    break;
            }
            ...
            ListaPedidosGridView.Columns[14].Visible = Convert.ToBoolean(PedidoClass.UsuCodCopia);
        }
        ...
        protected void btnVerDetalhe_Click(object sender, EventArgs e)
        {
            ObjFiltroClass = new FiltroClass();
            ObjFiltroClass.EmpCod = drpEmpresa.SelectedItem.Value;
            ObjFiltroClass.PedVendaStatDescr = drpListFiltroStat.SelectedValue;
            //ObjFiltroClass.PedVendaTipo = drpListFiltroTipo.SelectedItem.Text;
            ObjFiltroClass.DropOpcaoFiltro = drpListFiltroPri.SelectedValue;
            ObjFiltroClass.TextoFiltro = txtFiltro.Text;
            Session["ObjFiltroClass"] = ObjFiltroClass;
            ...
            //Verifica se pedido ainda não foi para o SAP Envia
            if (PedidoClass.NumeroEsbocoSAP == 0)
            {
                pedido novoPedido = new pedido();
                novoPedido.carregaDadosPedido(PedidoClass.EmpCod, PedidoClass.PedVendaNum);
                novoPedido.EnviaPedidoSAP();
            }
        ...
        protected void RastrearLinkButton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                objPedidoClass.NumeroNotaFiscal = ((LinkButton)((Control)sender).FindControl("NumeroNotaFiscalLinkButton")).Text ?? "0";

                if (((System.Web.UI.Control)sender).ID != "NumeroNotaFiscalLinkButton")
                {
                    if (objPedidoClass.NumeroPedidoSAP == 0)
                    {
                        ApresentaMensagem("O pedido não pode ser rastreado sem ter gerado NumeroPedidoSAP");
                    }
                    else if (objPedidoClass.NumeroNotaFiscal == "0")
                    {
                        ApresentaMensagem("O pedido não pode ser rastreado sem nota fiscal");
                    }
                    else
                    {
                        Session["PedidoRastrear"] = objPedidoClass;

                        Response.Redirect("~/listas/RastrearPedido.aspx?indmnu=5");
                    }
                }
            }
            catch (Exception ex)
        ...
        protected void RastrearModalLinkButton_Click(object sender, EventArgs e)
        {
            Response.Redirect("~/listas/RastrearPedido.aspx?indmnu=5");
        }       
    }
