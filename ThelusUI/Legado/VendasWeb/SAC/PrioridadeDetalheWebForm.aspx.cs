...
{
    public partial class PrioridadeDetalheWebForm : System.Web.UI.Page
    {
        ...
        {
            if (Session["PrioridadeDetalhe"] != null)
            {
                ObjSAC = (SACClass)Session["PrioridadeDetalhe"];
            }

            if (ObjSAC.Operacao == "Alteracao")
            {

                ObjSAC.Filtro = ObjSAC.IDPrioridade.ToString();
                CodigoTextBox.Text = ObjSAC.IDPrioridade.ToString();

                DataTable SACDataTable = new DataTable();

                ObjSAC.Tela = "Detalhe";
                SACDataTable = ObjSAC.RetornaListaPrioridades();
        ...
        protected string CarregaDadosDaTela()
        {
            ...
            if (erro == "")
            {
                ...
                {
                    ObjSAC.IDPrioridade = 0;
                }
                else
                {
                    ObjSAC.IDPrioridade = Convert.ToInt32(CodigoTextBox.Text);
                }
